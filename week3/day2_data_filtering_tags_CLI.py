...
def get_running_instances(data,environment):
    running_instances = []
    for reservation in data.get('Reservations', []):
        for instance in reservation.get('Instances',[]):
            ...
            if instance.get('State',{}).get('Name') != "running":
                ...
                # step 2: check if env is development/production
                is_env_match = False
...
def save_report(filename, running_instances, output_format):
    ...
    if output_format == "csv":
        with open(filename, 'w',newline='') as file:
            ...
            fieldnames =  ['instance_id', 'instance_type', 'state', 'private_ip','public_ip', 'environment']
...
def main():
    ...
    if data is None:
        ...
        return
    running_instances = get_running_instances(data,args.env)
